...
### Summmary
One of the easiest and most intuitive way to model time series data like Stock Prices, Text Words etc is a Markov
Chain. ChainoPy makes it easy for you to model time series data using models like Markov Chains, Markov Switching 
Models etc while taking computational aspects like speed and memory into account. Apart from that, you could also
model behavior of probablistic models like Markov Chains through Neural Networks which are widely considered 
deterministic using the `MarkovChainNeuralNetwork` class.
